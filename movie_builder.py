    ...
    def __init__(self,types,x,y,z):
        ...
        class Fibrils:
            ...
            def __init__(self):
                ...
                self.isolated = list(filter(lambda x: len(x)==1, self.clusters))
                self.alignment_matrices = []
                self.network = []
                self.nematic_cutoff = -1.0
        ...
        for identity,cluster in enumerate(fibrils.network+fibrils.isolated):
            for monomer in cluster:
                new_types[monomer*fibronectin_length:(monomer+1)*fibronectin_length] = identity+1
